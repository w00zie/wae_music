...
def encoder(config: dict, name: str = "musegan_encoder") -> tf.keras.Model:
    h_dim = config["h_dim"]
    z_dim = config["z_dim"]
    ...
    encoder = tf.keras.Sequential(
        [   
            ...
            tf.keras.layers.Conv2D(filters=16*h_dim, 
                                    ...
                                    padding="same"),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU(name='act_5'),
...
def decoder(config: dict, name: str = "musegan_decoder") -> tf.keras.Model:
    h_dim = config["h_dim"]
    z_dim = config["z_dim"]
    kernel_size = config["conv_kernel_size"]
    kernel_init = config["kernel_init"]
    ...
    decoder = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Reshape((1, 1, z_dim)),
            tf.keras.layers.Conv2DTranspose(filters=16*h_dim, 
            ...
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU(name='act_1'),
            tf.keras.layers.Conv2DTranspose(filters=8*h_dim, 
