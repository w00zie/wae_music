...
def encoder(config: dict, name: str ="first_encoder") -> tf.keras.Model:
...
def decoder(config: dict, name: str = "first_decoder") -> tf.keras.Model:
    ...
    decoder = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(units=12*7*(8*h_dim), 
                                    kernel_initializer=kernel_init),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU(),
            tf.keras.layers.Reshape((12, 7, 8*h_dim)),
